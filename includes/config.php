...
function getBaseUrl() {
    ...
    // Check if BASE_URL is defined in .env file
    $envFile = dirname(__DIR__) . DIRECTORY_SEPARATOR . '.env';
    if (file_exists($envFile)) {
        $envContents = file_get_contents($envFile);
        if (preg_match('/BASE_URL\s*=\s*([^\s]+)/i', $envContents, $matches)) {
    ...
    // Check if there's a config.php in the root directory
    $rootConfigFile = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'config.php';
    if (file_exists($rootConfigFile)) {
        // Include the file and check if BASE_URL is defined
        include_once $rootConfigFile;
    ...
    // Better handling for subdirectory installations
    // First check if we're in a subdirectory by examining REQUEST_URI and SCRIPT_NAME
    if (isset($_SERVER['REQUEST_URI']) && isset($_SERVER['SCRIPT_NAME'])) {
        // Get the directory part of SCRIPT_NAME
        $scriptPath = dirname($_SERVER['SCRIPT_NAME']);
        
        // If we're in the root directory, dirname returns '\' on Windows or '/' on Unix
        // Normalize to '/'
        $scriptPath = str_replace('\\', '/', $scriptPath);
        
        // If we're in the root directory, set to '/'
        if ($scriptPath == '.') {
            $scriptPath = '/';
        }
        
        // Ensure we have a trailing slash for non-root directories
        if ($scriptPath != '/') {
        ...
        // Fallback if REQUEST_URI or SCRIPT_NAME is not available

    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['pdf_backup_csrf_token']) {
        ...
        try {
            ...
            if (count($documents) === 0) {
                ...
                $pdf->SetCreator('PDF Link');
                $pdf->SetAuthor('PDF Link System');
                $pdf->SetTitle('PDF Link Backup');
                $pdf->SetSubject('PDF Link Document Backup');
                $pdf->SetKeywords('PDF, QR, Backup');
                
                // Set default header data
                $pdf->SetHeaderData('', 0, 'PDF Link Backup', 'Generated on ' . date('Y-m-d H:i:s'));
                ...
                // Title
                $pdf->Cell(0, 10, 'PDF Link System Backup', 0, 1, 'C');
